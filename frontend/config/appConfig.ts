...
export const appConfig = {
  brandName: process.env.NEXT_PUBLIC_BRAND_NAME ?? "EAC",
  brandSubtitle: process.env.NEXT_PUBLIC_BRAND_SUBTITLE ?? "Capital Labor Intelligence",
  brandImageSrc: process.env.NEXT_PUBLIC_BRAND_IMAGE_SRC ?? "",
  apiBaseUrl: process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://127.0.0.1:8001",
  appTitle: process.env.NEXT_PUBLIC_APP_TITLE ?? "Employee Activity Classification Dashboard",
  appDescription:
  ...
  personaName: process.env.NEXT_PUBLIC_PERSONA_NAME ?? "Network Engineering Annual Survey",
  reviewThreshold: numberFromEnv(process.env.NEXT_PUBLIC_REVIEW_THRESHOLD, 68),
