...
class LLMClient:
    ...
    def available(self) -> bool:
        ...
        if self.settings.llm_provider == "azure_openai":
            return bool(self.settings.llm_api_base_url and self.settings.llm_api_key)
        if self.settings.llm_provider in {"openai", "openai_compatible"}:
            return bool(self.settings.llm_api_key)
        return bool(self.settings.llm_api_key or self.settings.llm_api_base_url)
    ...
    def available_async(self) -> bool:
        return litellm_acompletion is not None and self.available
    ...
    def _stub_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "investment_signals": [str(context.get("VZZ_CostClass") or "unknown cost class")],
            "org_context": f"Activity in {context.get('Team') or 'unassigned team'}",
            "classification_hints": [str(context.get("Activity_Type") or "unclassified")],
            "enrichment_notes": "Stub context — no LLM provider configured.",
        }
    ...
    def _stub_retrieval(self, context: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            "similarity_score": 0,
            "retrieval_notes": "Stub retrieval — no LLM provider configured.",
        }

    def _stub_policy(self, record: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "policy_verdict": "uncertain",
            "policy_rationale": "Stub policy reasoning — no LLM provider configured.",
            "applicable_rules": [],
    ...
    def _stub_response(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        summary = f"{payload.get('Activity_Type') or 'Activity'} in {payload.get('Team') or 'unassigned team'}"
        return {
            "summary": summary,
            "risk_flags": [],
            "suggested_tags": [str(payload.get("LaborSurvey_Category") or "uncategorized")],
            "evidence_note": "Local stub reasoning used because no LLM provider was configured.",
