...
class Settings:
    app_name: str = os.getenv("APP_NAME", "Employee Activity Classification API")
    database_url: str = os.getenv(
        "DATABASE_URL",
        f"sqlite:///{(BACKEND_ROOT / 'eac.db').as_posix()}"
    )
    rule_version: str = os.getenv("RULE_VERSION", "fixed-asset-policy-v1.0")
    persona_name: str = os.getenv("PERSONA_NAME", "Network Engineering Annual Survey")
    review_threshold: int = _as_int(os.getenv("REVIEW_THRESHOLD"), 68)
